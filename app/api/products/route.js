...
export async function GET(request) {
    ...
    const per_page = searchParams.get('per_page') || 100; // Default to 100 products instead of 10
    ...
    try {
        ...
        const totalProducts = response.headers['x-wp-total'] || response.data.length;
        const totalPages = response.headers['x-wp-totalpages'] || 1;

        // Process products to add additional metadata about attributes
        const processedProducts = response.data.map(product => {
            // Check if product has attributes that require selection
            const hasRequiredAttributes = product.attributes && product.attributes.length > 0;

            // Categorize attributes by type for easier frontend handling
            const categorizedAttributes = {};
            if (hasRequiredAttributes) {
                product.attributes.forEach(attr => {
                    // Convert attribute name to lowercase for consistency
                    const attrName = attr.name.toLowerCase();
                    categorizedAttributes[attrName] = {
                        name: attr.name,
            ...
            return {
                ...product,
                requires_attribute_selection: hasRequiredAttributes,
                categorized_attributes: categorizedAttributes
            };
        });
